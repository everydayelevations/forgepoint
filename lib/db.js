...
async function ensureSchema() {
  ...
  `
  await sql`ALTER TABLE verifications ADD COLUMN IF NOT EXISTS sales_rep_email TEXT`
  await sql`ALTER TABLE verifications ADD COLUMN IF NOT EXISTS sales_rep_raw   TEXT`
...
export async function saveVerification(r) {
  ...
  const rows = await sql`
    INSERT INTO verifications (
      job_address, vendor, invoice_number, so_number, sales_rep, sales_rep_email, sales_rep_raw,
      total_lines, matched, issues, missing, match_rate, overall_status,
      report, trim_data
    ) VALUES (
      ${r.jobAddress}, ${r.vendor}, ${r.invoiceNumber}, ${r.soNumber},
      ${r.salesRep}, ${r.salesRepEmail}, ${r.salesRepRaw},
